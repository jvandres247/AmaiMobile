...
const IntentionScreen: React.FC<Props> = ({navigation}) => {
  const {setGoalIds} = useOnboarding();

  const handleMultipleSelection = (values: string[]) => {
    console.log('Desde el padre (función):', values);
    setGoalIds(values);
  };
  const {emotionalGoals, loading, error} = useEmotionOnboarding();
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.altButtons}>
          ...
          <Button
            ...
            onPress={() => navigation.navigate('ManageEmotionScreen')}
          />
        </View>
      </SafeAreaView>
    </ScreenLayout>
